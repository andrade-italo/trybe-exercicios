...
/* 4 - Suponha que você precise simular uma mensagem enviada do robô Curiosity de 
...
sucesso a temperatura em Marte. Logo, adicione na função sendMarsTemperature uma 
...
const sendMarsTemperature = (funcao) => {
    return setTimeout(() => funcao(getMarsTemperature()), messageDelay())
} 
...
devido ao fato de o robô já estar muito ocupado com outras operações. Logo, adicione 
na função sendMarsTemperature uma outra callback que contenha as ações a serem tomadas em caso de falha. */
const messageDelay = () => Math.floor(Math.random() * 5000);
...
const sendMarsTemperature = (sucess,fail) => {
    Math.random() >= 0.4? sucess(getMarsTemperature()) : fail('Robot is busy') 
}
