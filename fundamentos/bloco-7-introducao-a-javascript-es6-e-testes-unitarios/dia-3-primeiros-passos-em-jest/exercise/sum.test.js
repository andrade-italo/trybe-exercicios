...
describe("4 - Para as funções encode e decode crie os seguintes testes em Jest:", () => {
  ...
  it("Teste se as demais letras/números não são convertidos para cada caso;", () => {
    ...
    expect(decode("8,9,0")).toBe("8,9,0");
  });
...
describe("Testa a função searchEmployee", () => {
  it("Teste para checar se a função existe", () => {
    expect(searchEmployee).toBeDefined();
  });
  it("Testa se o retorno do id 4678-2 e firstName é paul", () => {
    expect(searchEmployee("4678-2", "firstName")).toBe("Paul");
  });
  it("Testa se a mensagem de erro de um id inexistente é igual a 'ID não identificada'", () => {
    expect(() => searchEmployee("544", "firstName")).toThrowError(
      "ID não identificada"
    );
  });
  it("Testa se a mensagem de erro de uma especialidade inexistente é igual a 'Informação indisponível'", () => {
    expect(() => searchEmployee("4678-2", "goiabada")).toThrowError(
